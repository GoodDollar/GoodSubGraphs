...
import { UBIScheme, UBICalculated, UBIClaimed, ActivatedUser, InactiveUserFished } from '../generated/UBIScheme/UBIScheme'
...
export function handleUBICalculated(event: UBICalculated): void {
  ...
  let gd = GoodDollar.bind(Address.fromString(dataSource.context().getString('gdTokenAddress')))
...
function aggregateDailyUbiFromUBIClaimed(event: UBIClaimed): void {
  ...
  if (dailyUbi) {
    ...
    if (enableLogs)
      log.info('handleUBIClaimed dailyUbi.id {}, dailyUbi.totalUBIDistributed {}, dailyUbi.newClaimers {}, contract address {}', [
        dailyUbi.id.toString(),
        dailyUbi.totalUBIDistributed.toString(),
        dailyUbi.newClaimers.toString(),
